...
const SERVER_HOSTNAME = process.env.SERVER_HOSTNAME,
  ...
  localhost = `http://${SERVER_HOSTNAME}:${SERVER_PORT}`;
...
let htmlContentTemplate = readFileSync(
    modifiedRoute + "public/ContentBody.html",
    "utf-8"
  ),
  htmlGetResponseTemplate = readFileSync(
    modifiedRoute + "public/GETS/GETS.html",
    "utf-8"
  ),
  htmlPostResponseTemplate = readFileSync(
    modifiedRoute + "public/POST/POST.html",
    "utf-8"
  ),
  htmlUpdateResponseTemplate = readFileSync(
    modifiedRoute + "public/UPDATE/UPDATE.html",
    "utf-8"
  ),
  htmlDeleteResponseTemplate = readFileSync(
    modifiedRoute + "public/DELETE/DELETE.html",
    "utf-8"
  );

let returnBackButton = `<button><a href="/">BACK</a></button>`,
  columnsName = ["Nome", "Cognome", "Indirizzo", "Posta"];
...
const modelliDB = new SQLite3.Database(
  join(modifiedRoute + "./Database/modelli.db")
);
  ...
  modelliDB.serialize((_) => {
    modelliDB.all("SELECT ROWID, * FROM Modelli", (err, rows) => {
      ...
      res
        .status(200)
        .send(
          htmlGetResponseTemplate.replace(
            "{{%Content%}}",
            returnBackButton + replacedRows
          )
        );
    });
  });
...
crud.get("/Nome=:Nome", (req, res) => {
  ...
  modelliDB.serialize((_) => {
    modelliDB.all(
      "SELECT ROWID, * FROM Modelli WHERE Nome = $Nome",
      {
        $Nome: nome,
      },
      (err, rows) => {
...
crud.post("/", (req, res) => {
  ...
  modelliDB.serialize((_) => {
    modelliDB.run(
      "INSERT INTO Modelli(Nome, Cognome, Indirizzo, Posta) VALUES ($Nome, $Cognome, $Indirizzo, $Posta)",
      jsonObject,
      (err) => {
        if (err) {
          const errorObj = {
...
crud.get("/Id=:id", (req, res) => {
  ...
  modelliDB.serialize((_) => {
    modelliDB.all(
      "SELECT ROWID, * FROM Modelli WHERE ROWID = $Id",
      {
        $Id: id,
      },
      (err, row) => {
        ...
        let replacedRow = rowsRefinment(replaceRows(row)),
          getElementsRow = replacedRow.split("\n"),
          content = [];
...
crud.get("/update/Id=:id", (req, res) => {
  const fullUrl = req.url,
    urlParams = fullUrl.split("?")[1].split("&");
  let id = req.params.id,
    jsonObject = {
      $Id: id,
    };
  ...
  modelliDB.serialize((_) => {
    modelliDB.run(
      ...
      async (err) => {
        ...
        await fetch(`${localhost}/CRUD/update/Id=${id}`, options).then(
...
crud.get("/delete/Id=:id", (req, res) => {
  ...
  modelliDB.serialize((_) => {
    modelliDB.run(
      "DELETE FROM Modelli WHERE ROWID = $Id",
      {
        $Id: id,
      },
      (err) => {
        ...
        fetch(`${localhost}/CRUD/delete/Id=${id}`, options).then(
          res.status(200).send(htmlDeleteResponseTemplate + returnBackButton)
        );
      }
    );
  });
...
function replaceRows(rows) {
  let jsonTemplate = JSON.parse(JSON.stringify(rows)),
    replacedRows = jsonTemplate.map((json) => {
